function [model, varargout] = GRLVQ_train(trainSet, trainLab, varargin)
%GRLVQ_trai.m - trains the Generalized Relevance LVQ algorithm
...
switch(p.Results.optimization)
case{'sgd'}
    DiffMinChange = initialization.DiffMinChange;
    ...
    elseif length(p.Results.learningRateRelevances)>2
        if length(p.Results.learningRateRelevances)==nb_epochs
            ...
            disp('The learning rate vector for the relevances does not fit the nb of epochs');
            return;
        end
    else
        % or use an decay with a start and a decay value
