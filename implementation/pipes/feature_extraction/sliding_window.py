...
from interpolation.cheb_interpolation import ChebInterpolation
...
class SlidingWindow(object):
    def __init__(self, window_size=10, nr_of_coefs=10, nr_of_samples=25, omit_first_coef=True, file_path=None):
        ...
        self.polyInt = ChebInterpolation()
    ...
    def interpolate(self, meas_df):
        ...
        coefs = self.polyInt.find_coefs(self.nr_of_coefs, t, y)
        first_coef = coefs[0]
        if self.omit_first_coef:
            coefs[0] = 0
        x = np.arange(-1, 1, (2. / self.nr_of_samples))  # -1 and 1 see chebs paper
        y_hat, _ = self.polyInt.get_y_hat_range(coefs, x)
